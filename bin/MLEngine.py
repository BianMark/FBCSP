...
class MLEngine:
    ...
    def experiment(self, experiment_details):
        ...
        if experiment_details['classifier']['classifier_type'] == 'GaussianNB':
            classifier_type = GaussianNB(var_smoothing=experiment_details['classifier']['var_smoothing'])
        if experiment_details['classifier']['classifier_type'] == 'NBPW':
            classifier_type = NBPW()
        if experiment_details['classifier']['classifier_type'] == 'kNN':
            classifier_type = KNeighborsClassifier(n_neighbors=experiment_details['classifier']['k'])

        if experiment_details['feature_selection']['feature_selection_type'] == 'MIBIF':
            classifier = Classifier(classifier_type, n_features_select=experiment_details['feature_selection']['n_features_select'], n_csp_pairs=experiment_details['feature_selection']['n_csp_pairs'])

        for k in range(self.ntimes):
            if self.ntimes > 1:
                '''for N times K fold CV'''
...
class FilterBank:
    def __init__(self, fs):
        self.fs = fs
        self.f_trans = 2  # transition width (Hz)
        self.f_pass = np.arange(4, 40, 4)
        self.f_width = 4  # the width of each frequency sub-band (Hz)
        self.gpass = 3  # maximum loss in the passband (dB)
        self.gstop = 30  # minimum attenuation in the stopband (dB)
        self.filter_coeff = {}

    def get_filter_coeff(self):
        """
        Compute the numerator(denominator) polynomials of chebyshev 2 filter for each frequency sub-band. In professor Kai Keng Ang's paper, the frequency band should be 4:4:40.

        : returns self.filter_coeff: a dictionary contains the parameters of chebyshev 2 filter for each frequency sub-band,
            'i': the index of frequency sub-band,
            'b': the numerator polynomials,
            'a': the denominator polynomials
        """
        ...
        for i, f_low_pass in enumerate(self.f_pass):
            ...
            order, wn = cheb2ord(wp, ws, self.gpass, self.gstop)  # 'order' is the lowest order of the chebyshev filter
            b, a = signal.cheby2(order, self.gstop, ws, btype='bandpass')  # b(a) is the numerator(denominator) polynomials of chebyshev 2 filter
    ...
    def filter_data(self, eeg_data, window_details={}):
        """
        Decompose the eeg data into different frequency sub-bands.

        :param eeg_data: eeg data, (n_trials, n_channels, n_samples)
        :param window_details: start and end of the time window (after the cue), 'tmin', 'tmax'
        :returns filtered_data, eeg data for different frequency sub-bands, (n_fbanks, n_trials, n_channels, n_samples)
        """
